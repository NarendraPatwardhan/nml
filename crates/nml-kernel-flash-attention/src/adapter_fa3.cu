...
void launch(Flash_fwd_params &params, cudaStream_t stream) {
  run_mha_fwd_<90, Element, HeadDimension, HeadDimension, false, false, false,
               false>(params, stream);
}

template <typename Element>
void dispatch_head_dimension(Flash_fwd_params &params, cudaStream_t stream) {
  if (params.d <= 64) {
...
template <typename Element, int HeadDimension>
void launch_paged(Flash_fwd_params &params, cudaStream_t stream) {
  run_mha_fwd_<90, Element, HeadDimension, HeadDimension, false, true, false,
               true>(params, stream);
}

template <typename Element>
void dispatch_paged_head_dimension(Flash_fwd_params &params,
                                   cudaStream_t stream) {
  if (params.d <= 64) {
...
extern "C" int32_t
nml_flash_attention_3_forward(const NmlFlashAttentionForward *request,
                              char *error_message,
  ...
  if (request->query == nullptr || request->key == nullptr ||
      ...
      request->softmax_lse == nullptr || request->workspace == nullptr ||
      request->stream == nullptr) {
    return fail(kInvalidArgument,
                "FlashAttention 3 requires non-null buffers, workspace, and "
                "CUDA stream",
                error_message, error_message_capacity);
  }
  ...
  if (request->batch_size >
      ...
      request->batch_size >
          std::numeric_limits<int32_t>::max() / request->key_length) {
  ...
  try {
    ...
    if (status == cudaSuccess) {
      status = cudaDeviceGetAttribute(&multiprocessor_count,
    ...
    params.dv = request->head_dimension;
    params.total_q = request->batch_size * request->query_length;
    params.total_k = request->batch_size * request->key_length;
    ...
    params.num_splits = 1;
    params.page_size = 1;
    params.arch = capability_major * 10 + capability_minor;
    params.num_sm = multiprocessor_count;
    ...
    int window_right =
        request->causal != 0
            ? 0
            : (request->sliding_window < 0 ? -1 : request->sliding_window - 1);
    if (window_left >= request->key_length - 1) {
      window_left = -1;
    }
    if (window_right >= request->query_length - 1) {
      window_right = -1;
    }
    if (request->causal != 0) {
      window_right = 0;
    ...
    params.is_local =
        (window_left >= 0 || window_right >= 0) && !params.is_causal;
    params.window_size_left =
        window_left < 0 ? request->key_length - 1 : window_left;
    params.window_size_right =
        window_right < 0 ? request->query_length - 1 : window_right;

    // Upstream's fixed-length SM90 scheduler needs a semaphore only for
    // causal or local attention. Keep the caller-owned slot in the stable ABI,
    // but do not change the unmasked scheduler by passing a needless pointer.
    params.tile_count_semaphore = (params.is_causal || params.is_local)
                                      ? static_cast<int *>(request->workspace)
                                      : nullptr;

    auto stream = static_cast<cudaStream_t>(request->stream);
    if (params.tile_count_semaphore != nullptr) {
      status = cudaMemsetAsync(request->workspace, 0, sizeof(int32_t), stream);
      if (status != cudaSuccess) {
        return fail(kLaunchFailure, cudaGetErrorString(status), error_message,
                    error_message_capacity);
      }
...
extern "C" int32_t nml_flash_attention_3_paged_forward(
  ...
  if (request->query == nullptr || request->key_cache == nullptr ||
      ...
      request->stream == nullptr) {
    return fail(kInvalidArgument,
                "paged FlashAttention 3 requires non-null buffers, workspace, "
                "and CUDA stream",
                error_message, error_message_capacity);
  }
  if (request->num_pages <= 0 || request->page_size <= 0 ||
      request->max_pages_per_sequence <= 0 || request->batch_size <= 0 ||
      request->query_length <= 0 || request->query_heads <= 0 ||
      request->key_value_heads <= 0 || request->head_dimension <= 0 ||
      request->head_dimension > 256 || request->head_dimension % 8 != 0 ||
      request->query_heads % request->key_value_heads != 0) {
    return fail(kInvalidArgument, "unsupported paged FlashAttention 3 geometry",
                error_message, error_message_capacity);
  }
  if (request->sliding_window == 0 || request->sliding_window < -1 ||
  ...
  try {
    ...
    if (capability_major != 9 || capability_minor != 0) {
      return fail(
          kInvalidArgument,
          "paged FlashAttention 3 requires CUDA compute capability SM90",
          error_message, error_message_capacity);
    }
    ...
    params.o_row_stride = request->output_row_stride;
    params.o_head_stride = request->output_head_stride;
    params.page_table = static_cast<int *>(request->page_table);
    params.page_table_batch_stride = request->page_table_batch_stride;
    ...
    params.dv = request->head_dimension;
    params.total_q = request->batch_size * request->query_length;
    params.total_k = request->num_pages * request->page_size;
    params.seqlen_q_rounded = rounded_query_length;
    params.seqlen_k_rounded = rounded_key_length;
    params.d_rounded = request->head_dimension <= 128
                           ? (request->head_dimension + 31) / 32 * 32
    ...
    params.num_pages = request->num_pages;
    params.pagedkv_tma = false;
    params.pack_gqa = true;
    params.arch = 90;
    params.num_sm = multiprocessor_count;
    params.tile_count_semaphore = static_cast<int *>(request->workspace);
    ...
    int window_right =
        request->causal != 0
            ? 0
            : (request->sliding_window < 0 ? -1 : request->sliding_window - 1);
    if (window_left >= params.seqlen_k - 1) {
      window_left = -1;
    }
    if (window_right >= request->query_length - 1) {
      window_right = -1;
    ...
    params.is_local =
        (window_left >= 0 || window_right >= 0) && !params.is_causal;
    params.window_size_left =
        window_left < 0 ? params.seqlen_k - 1 : window_left;
    params.window_size_right =
        window_right < 0 ? request->query_length - 1 : window_right;
