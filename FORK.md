...
| D-039 | `DECIDED` | End-to-end text-model execution is now a product capability rather than an external caller exercise. NML retains ZML's IREE tokenizer choice for Hugging Face `tokenizer.json`, adapted behind one safe Rust `Tokenizer` owner and a narrow C ABI; IREE is pinned from its original repository and never imports a ZML Bazel target. Qwen3 is the first model contract: the dense architecture, tied embeddings, Q/K normalization, RoPE, causal GQA, SwiGLU, persistent per-layer KV state, prefill/decode split, and greedy generation are compiled through the existing StableHLO/XLA/PJRT substrate. The official `Qwen/Qwen3-0.6B` bf16 revision `c1899de289a04d12100db370d81485cdf75e47ca` is the initial end-to-end artifact. Model files remain ignored local inputs whose revision and SHA-256 values are verified; NML does not add a Hugging Face network filesystem or a second model format. CPU is the mandatory local bf16 execution gate, while the architecture remains backend-neutral and later CUDA execution follows the existing hardware-evidence rules. |
...
Hosted Workflows have a second, independent image choice: the coordinator VM.
BuildBuddy's implicit workflow uses Ubuntu 18.04 and invokes `bazel test //...`
without NML's `buildbuddy` profile. That combination can reuse an unqualified
native build-tool output produced on a newer-glibc host and then fail before
any product test runs. `buildbuddy.yaml` therefore pins the coordinator to the
same Ubuntu 20.04 ABI generation and spells `--config=buildbuddy` on every
invocation. It calls `//:cpu_contracts`, `//:cuda_remote_contracts`,
`//:cuda_package_contracts`, and `//:cuda_contract_binaries`; it never schedules
`//:cuda_device_contracts` on a runner with no NVIDIA device. The workflow image
and the RBE execution property solve different halves of the same portability
boundary, so neither may be removed merely because the other exists.
